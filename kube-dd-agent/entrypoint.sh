...
for i in `ls /etc/dd-agent/conf.d`
do
    NEW=`echo $i|tr '-' '_'`
    mv /etc/dd-agent/conf.d/$i /etc/dd-agent/conf.d/$NEW
done
for i in `ls /etc/dd-agent/checks.d`
do
    NEW=`echo $i|tr '-' '_'`
    mv /etc/dd-agent/checks.d/$i /etc/dd-agent/checks.d/$NEW
done
